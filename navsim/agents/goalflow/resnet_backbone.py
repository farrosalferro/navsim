...
class ResNetBackbone(nn.Module):
    ...
    def fuse_features(self, image_features, lidar_features, layer_idx):
        ...
        """
        image_embd_layer = self.avgpool_img(image_features) # [bz,64,8,32]
        lidar_embd_layer = self.avgpool_lidar(lidar_features) # [bz,64,8,8]

        lidar_embd_layer = self.lidar_channel_to_img[layer_idx](lidar_embd_layer) # [bz,64,8,8]

        image_features_layer, lidar_features_layer = self.transformers[layer_idx](
            image_embd_layer, lidar_embd_layer
        )  # [bz,64,8,32] [bz,64,8,8]
        lidar_features_layer = self.img_channel_to_lidar[layer_idx](lidar_features_layer) #[bz,64,64,64]
        ...
        image_features = image_features + image_features_layer # [bz,64,64,256]
        lidar_features = lidar_features + lidar_features_layer # [bz,64,64,64]
